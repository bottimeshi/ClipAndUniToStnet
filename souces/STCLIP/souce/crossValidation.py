...
def train_model(model, data_loader, criterion, optimizer, num_epochs, batch_size):
    for epoch in range(num_epochs):
        ...
        for batch_X, batch_y in data_loader:
            if batch_X.size(0) == 1:
                continue
            batch_X, batch_y = batch_X.to(device), batch_y.to(device)  # デバイスに移動
            # 順伝播
...
def cross_validation(inputX_dir, inputY_dir, input_dim, output_dim, batch_size, num_epochs, learning_rate, momentum):
    ...
    for i, test_patient in enumerate(spots.keys()):
        ...
        results.append((test_patient, y_pred, y_test.cpu()))

    return results

def main():
    inputX_dir = "STCLIP/data/forTraining/imageFeatures"
    inputY_dir = "STCLIP/data/forTraining/countsData"
    output_dir = "STCLIP/output/cross"  # pthファイルの保存先ディレクトリ
    input_dim = 512
    output_dim = 250
    batch_size = 32
    num_epochs = 100
